...
object OntContructorMain {
  def main(args: Array[String]): Unit = {
    ...
    val ONTOLOGYURI="http://www.semanticweb.org/rashmi/ontologies/2017/7/"

    val manager = OWLManager.createOWLOntologyManager
    //creating ontology manager
    val df = manager.getOWLDataFactory //In order to create objects that represent entities

    val ontology = manager.createOntology(IRI.create(ONTOLOGYURI,"rashmi#"))
    //Prefix for all the entities
    val pm = new DefaultPrefixManager(null, null, ONTOLOGYURI+"rashmi#")
    ...
    val os = new FileOutputStream("data/tensorflownew.owl")
    val owlxmlFormat = new OWLXMLDocumentFormat
    manager.saveOntology(ontology, owlxmlFormat, os)
    System.out.println("Ontology Created for my dataset i.e tensorflow")

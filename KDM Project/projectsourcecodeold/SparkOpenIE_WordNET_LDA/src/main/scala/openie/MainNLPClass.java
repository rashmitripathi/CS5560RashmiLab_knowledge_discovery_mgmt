...
public class MainNLPClass {
    ...
    public static String returnTriplets(String sentence) throws IOException {
        ...
        FileWriter fileSubject = new FileWriter("myClassRashmki", true);
        FileWriter fileTriplets = new FileWriter("TripletsNewRashmi", true);
        FileWriter fileData = new FileWriter("DataPropertiesRashmi", true);
        FileWriter fileIndividual = new FileWriter("IndividualsRashmi", true);
        FileWriter temp = new FileWriter("tempRashmi", true);
        FileWriter myClass = new FileWriter("ClassRashmi", true);
        FileWriter objectFile = new FileWriter("ObjectPropertiesRashmi", true);
        ...
        for (Sentence sent : doc.sentences()) {  // Will iterate over two sentences
            ...
            for (Quadruple x : l) {
                ...
                if (!subjectNER.equals("O")) {
                    ...
                    if (subject.contains("music") || subject.contains("sing") || subject.contains("Singer")) {
                        fileIndividual.write("MUSIC," + subject.replaceAll(" .*$", "") + "\n");
                    } else if (subject.contains("Karaoke")) {
                        fileIndividual.write("SINGER," + subject.replaceAll(" .*$", "") + "\n");
                    }
                }
                ...
                if ((subject.contains("music") || subject.contains("sing") || subject.contains("singer")) &&
                        (object.contains("music") || object.contains("sing") || object.contains("singer"))) {

                    objectFile.write(predicate + ",MUSIC,MUSIC,Func\n");

                }
                if ((subject.contains("music") || subject.contains("sing") || subject.contains("singer")) &&
                        (object.contains("Karaoke"))) {

                    objectFile.write(predicate + ",MUSIC,SINGER,Func\n");

                }
                if ((subject.contains("Karaoke")) &&
                        (object.contains("music") || object.contains("sing") || object.contains("singer"))) {
                ...
                if ((subject.contains("Karaoke")) &&
                    ...
                    objectFile.write(predicate + ",SINGER,SINGER,Func\n");
    ...
    public static HashSet<String> getSynonyms(String word) {
        RiWordNet wordnet = new RiWordNet("E:\\KDM\\Lab 7\\WordNet-3.0");
    ...
    public static void tripletGeneration(String subject, String predicate, String object) throws IOException {
        ...
        FileWriter fileWriter = new FileWriter("TripletGeneratedRashmi", true);
        String subjectNER = returnNER(subject);
        String objectNER = returnNER(object);
        HashSet<String> subjectSynonym = new HashSet<>();
        HashSet<String> objectSynonym = new HashSet<>();
        if (subject.contains("Music")) {
            String x = subject.replaceAll(".+", "Music");
            subject = x;
        }
        if (subject.contains("sing")) {
            String x = subject.replaceAll(".+", "sing");
            subject = x;
        }
        if (subject.contains("Karaoke")) {
            subject = subject.replaceAll(".+", "Band");
        }
        if (object.contains("Jazz")) {
            object = object.replaceAll(".+", "Jazz");
        }
        if (object.contains("lesson")) {
            object = object.replaceAll(".+", "lesson");
        }
        if (object.contains("coldplay")) {
            object = object.replaceAll(".+", "coldplay");
        }
        if (object.contains("Almoravides")) {
            object = object.replaceAll(".+", "Almoravides");
        }

...
object SparkNaiveBayes {
  ...
  private def run(params: Params) {
    ...
    val (inputVector, corpusData, vocabArrayCount) =
      preprocessWithoutNLP(sc, params.input)
  ...
  private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[Vector], RDD[(String,String)], Long) = {
    ...
    tfidf1.cache().saveAsTextFile("NaiveBayesFeatureVectorWithTFIDF")
  ...
  /**
    ...
    * @return (corpus, vocabulary as array, total token count in corpus)
    */
  private def preprocessWithoutNLP(sc: SparkContext,paths: Seq[String]): (RDD[Vector], RDD[(String,String)], Long) = {
    ...
    //Creating Inverse Document Frequency
    val tfidf1 = idf.transform(tf)
    tfidf1.cache().saveAsTextFile("NaiveBayesFeatureVectorWithoutNLP")

...
/**
  ...
  */
object SparkKMeansMainModified {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //tutorial data
    //val inputPath=Seq("data/20_news_group/*")

    //my data
    val inputPath=Seq("data/thesis/*")
    Logger.getRootLogger.setLevel(Level.WARN)

    val topic_output = new PrintStream("data/result_kmean.txt")
    // Load documents, and prepare them for KMeans.
    val preprocessStart = System.nanoTime()
    val (corpusVector, data, vocabSize) = preprocessNoNLP(sc, inputPath)
  ...
  private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], RDD[(String,String)], Long) = {
    ...
    tfidf1.cache().saveAsTextFile("tfVectorsForTensorflow")
  ...
  /**
    ...
    * @return (corpus, vocabulary as array, total token count in corpus)
    */
  private def preprocessNoNLP(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], RDD[(String,String)], Long) = {
    ...
    //Creating Term Frequency of the document
    val tf = hashingTF.transform(dfseq)
    tf.cache().saveAsTextFile("tfvectorsForTensorflow")

...
object SparkLDAMain {
  ...
  private def run(params: Params) {
    ...
    /*val (corpus, vocabArray, actualNumTokens) = preprocessAll(sc, params.input)
    corpus.cache()*/

    /*iii.	Data=>NLP=>StopWord=>LDA*/
      /*val (corpus, vocabArray, actualNumTokens) = preprocessNLPStopword(sc, params.input)
      corpus.cache()*/

    /*ii.	Data=>NLP=>LDA*/
      /* val (corpus, vocabArray, actualNumTokens) = preprocessNLP(sc, params.input)
      corpus.cache()*/

    //i. data => LDA
     val (corpus, vocabArray, actualNumTokens) = preprocessData(sc, params.input)
     corpus.cache()
  ...
  /**
    ...
    */
  private def preprocessAll(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) = {

    //this is Data=>NLP=>StopWord=>TFIDF=>LDA
    //Reading Stop Words
  ...
  private def preprocessNLPStopword(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) =
  {
    ...
    //val tfidf = idf.transform(tf)
     val a= tf.zipWithIndex().map(_.swap)

    val dff= stopWordRemovedDF.flatMap(f=>f)
    val vocab=dff.distinct().collect()
    (a, vocab, dff.count()) // Vector, Vocab, total token count
  }

  private def preprocessNLP(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) =
  {
    //this is Data=>NLP=>LDA
    //Reading Stop Words
    val df = sc.textFile(paths.mkString(",")).map(f => {
      val lemmatised=CoreNLP.returnLemma(f)
      val splitString = lemmatised.split(" ")
    ...
    //Creating an object of HashingTF Class
    val hashingTF = new HashingTF(dfseq.count().toInt)  // VectorSize as the Size of the Vocab
  ...
  private def preprocessData(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) = {
